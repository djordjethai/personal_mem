...
app_id = "test" # ime aplikacije
current_date = datetime.now()
date_string = current_date.strftime('%Y%m%d')

# Function to save personal facts using Pinecone
def save_facts(user_input, user_id, namespace, fact_type, app_id):
    ...
    query_response = index.query(
        ...
        namespace=namespace,
        top_k=1,
        filter={"user_id": user_id, "type": fact_type, "app_id": app_id},
...
def retrieve_facts(query, user_id, namespace, fact_type, app_id):
...
if response.choices[0].message.tool_calls is not None:
    for tool_call in response.choices[0].message.tool_calls:
        ...
        if tool_call.function.name == 'save_facts':
                ...
                save_facts(user_input, user_id, namespace, fact_type, app_id)
                print("!!!!!!!!!")
        ...
        if tool_call.function.name == 'retrieve_facts':
            fact_dict = json.loads(tool_call.function.arguments)
            fact_type= fact_dict['fact_type']
            ret_facts.append(retrieve_facts(user_input, user_id, namespace, fact_type, app_id))
            print("!!!!!!!!!")
